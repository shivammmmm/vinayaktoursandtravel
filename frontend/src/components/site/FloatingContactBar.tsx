...
import { MessageCircle, Bot, X, Send, Sparkles, Phone } from "lucide-react";
  ...
  const handleOptionClick = (option: string) => {
    ...
    setTimeout(() => {
      ...
      if (cleanOpt.includes("plan") || cleanOpt.includes("trip")) {
        ...
        replyOptions = ["🔙 Back to main menu"];
      } else if (cleanOpt.includes("human") || cleanOpt.includes("connect")) {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (cleanInput.includes("hi") || cleanInput.includes("hello") || cleanInput.includes("hey")) {
        replyText = "Hello! How can I help you plan your next trip today?";
      } else if (cleanInput.includes("price") || cleanInput.includes("cost") || cleanInput.includes("rate") || cleanInput.includes("package")) {
        replyText = "Our packages start from ₹12,999 (Domestic) & ₹29,999 (International). All itineraries are fully customizable! Which destination are you planning for?";
      } else {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2.5 max-w-[calc(100vw-2rem)]">
        ...
        <div className="w-[min(370px,calc(100vw-2rem))] overflow-hidden rounded-3xl border border-border bg-card shadow-brand flex flex-col h-[460px] animate-in fade-in slide-in-from-bottom-5 duration-300">
          ...
          <div className="flex items-center justify-between bg-primary px-4 py-3.5 text-primary-foreground">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="text-xs font-black tracking-tight">Vinayak Assistant</div>
                <div className="text-[10px] opacity-80 flex items-center gap-1">
                  <span className="h-1.5 w-1.5 rounded-full bg-green-400 animate-pulse" /> Online · 24×7
                </div>
              </div>
            </div>
            <button
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-0.5">
                    {m.options.map((opt) => (
                      <button
                        ...
                        className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-white border border-border text-foreground hover:border-accent hover:text-accent shadow-sm transition-all"
                      >
                        {opt}
                      </button>
                    ))}
      ...
      <div className="flex items-center gap-2 rounded-full border border-white/20 bg-primary/95 p-1.5 text-primary-foreground shadow-brand backdrop-blur-xl">
        ...
        >
          <Bot className="h-4 w-4 text-accent" />
          <span className="hidden sm:inline">Ask FAQ Bot</span>
        </button>

        {/* Plan Trip Link */}
        <Link
          to="/booking"
          className="flex items-center gap-1.5 rounded-full bg-accent px-3 py-1.5 text-xs font-black text-accent-foreground hover:brightness-110 transition-all shadow"
        >
